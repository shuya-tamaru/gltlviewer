import { Button, AlertDialogOverlay, AlertDialogHeader, AlertDialogContent, AlertDialogBody, AlertDialogFooter, AlertDialog, useToast } from '@chakra-ui/react';
...
export default function ({ index, isOpenAlert, onCloseAlert, comment, guid, setComments }: Props) {
  ...
  useEffect(() => {
    const getExistingImagePaths = async () => {
      const paths: string[] = await axios.get(`${process.env.NEXT_PUBLIC_LOCAL_PATH}/images/${comment.id}`).then(res => res.data);
      setImgePath(paths);
    }
    ...
    const deleteComment = async () => {
      if (currentUser) {
        try {
          ...
          imagePaths.length > 0
            ? deleleImageAndComment(comment.id)
            : deleteComment();
